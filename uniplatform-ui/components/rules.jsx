...
class RuleManage extends Component{
    ...
    fetchNodeList(){
        ...
        .then((data) => {
            ...
            // }
             if((data.nodes.length==0||special==true)&&specialFirst==true){
                let pageBool=true;
                if(this.state.colorIndex!=="Tem"){
                    special=false;
                }
                let colorIndex="Tem";
                this.setState({ nodeData,total0,colorIndex,pageBool,special},()=>this.fetchNodeRulesAll());
            }else{
                ...
                let nodeRulesDataAll=[];
                this.setState({ nodeData,total0,nodeId,pageBool,nodeRulesDataAll,addRuleId,specialFirst},()=>this.fetchNodeRules());
            }
        }).catch((err) => console.log(err.toString()));
    }
    //获取节点规则
        ...
        let param = new FormData();
         ...
         fetch(Uniplatform.context.url + '/nlap/admin/ruleMgmt/deleteRule', {
            ...
            .then((data) => {
                ...
                // data.status == '200' ? popup(<Snackbar message={data.msg}/>) + this.fetchRules() : popup(<Snackbar
                // message={data.msg}/>)
                data.status == '200' ? popup(<Snackbar message={data.msg}/>) + this.searchRules() : popup(<Snackbar
                message={data.msg}/>)
            }).catch((err) => {
                    ...
                    <Modal visible={ addTemVisible } size="middle" onClose={ this.closeAddTem.bind( this ) }>
                        ...
                        <ModalBody>
                            ...
                            <FormItem>
                                ...
                                {
                                    addTemId==0?
                                    <Select placeholder="请输入模板名称" search={ true } name="dicTypeId" showClear={ false }>
                                        {
