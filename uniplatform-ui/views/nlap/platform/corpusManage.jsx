...
class CorpusManage extends Component{
    ...
    fetchCorpusTypes(){
        ...
        .then((data) => {
            ...
            this.setState({corpusTypes},()=>this.clickCorpusTypes(0));
    ...
    //点击语料类型
    clickCorpusTypes(index1){
        if(index1==1||index1==2){
            return;
        }
        let index=3;
        let {corpusTypesIndex,corpusTypes,corpusTypeId,indexCorpusSet,indexObject,colorIndex,indexCorpus,pageStart,showCorpusSetBool,searchTableBool,sizeCorpusSet}=this.state;
        ...
        corpusTypes[index1].status=true;
        searchTableBool=false;
        sizeCorpusSet=9;
        this.setState({corpusTypes,corpusTypeId,corpusTypesIndex,indexCorpusSet,indexObject,colorIndex,indexCorpus,pageStart,showCorpusSetBool,searchTableBool,sizeCorpusSet},()=>this.getFunctions());
    }
    //获取功能
    getFunctions(){
    ...
    //返回
    quitObjectShow(){
        let {colorIndex,indexObject,indexCorpus,pageStart,objectBool,showCorpusSetBool,sizeCorpus,searchTableBool}=this.state;
        indexObject=1;
        colorIndex='Classify';
        indexCorpus=1;
        pageStart=0;
        ...
        sizeCorpus=10;
        searchTableBool=false;
        this.setState({colorIndex,indexObject,indexCorpus,pageStart,objectBool,showCorpusSetBool,sizeCorpus,searchTableBool},()=>this.fetchCorpusSetPage());
                                        ...
                                        return(
                                            <div key={index} style={{backgroundColor:`${corpusTypes[index].status?'#ceeefc':'#fff'}`,border:`${corpusTypes[index].status?'solid 1px #6fccf5':'1px solid #e4e4e4'}`}} 
                                                ...
                                                <span className="cortype_text" style={{color:'#da8b5c'}}>
                                                    {corpusTypes[index].name}<br/>({`${(index==0||index==3)?corpusTypes[3].corpusNum:0}`})
                                                </span>
                                            </div>
                                        )
                                    })
