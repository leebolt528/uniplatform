...
class StrategyManage extends Component {
    ...
    beforeSubmit(data){
        let {isRules,isModals,isDic} = this.state;
    ...
    beforeUpdateSubmit(data){
        let {isDic} = this.state;
        console.log(isDic)
        console.log(data.dictsId)
        let newName = data.strategyNewName.replace(/(^\s*)|(\s*$)/g, "");
        if(!newName){
            popup(<Snackbar message="策略名不能为空"/>);
            return false;
        }else if(!isDic && (data.dictsId.length==0)){
            popup(<Snackbar message="词库不能为空"/>);
            return false;
        }else {
            let depStr = data.dependencyStrategyId;
                                    ...
                                    <Col size={{ normal: 24, small: 24, medium: 24, large: 24 }}>
                                        ...
                                        <Table  striped={true} multiLine={true} checkable headBolder style={{color:'#54698d'}}
                                            ...
                                            <Column title="策略名称" scaleWidth='15%' textAlign="center">
                                                {(value)=>{
                                                    return(
                                                        <p><span style={{overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',width:'100px',display:'inline-block'}}>
                                                            {value.name}&nbsp;
                                                        </span>
                                                            {value.defaultUse ?
                                                                <span style={{backgroundColor:'#8fb5dd',fontSize:'8px',padding:'2px',float:'right'}}>默认</span>
                                                                : undefined}
                                                        </p>
                                                    )
                                                }}
                                                ...
                                                {(value)=>{
                                                    ...
                                                    switch (value.inUsing){
                                                        case 0:
                                                            ...
                                                            break;
                                                        case 1:
                                                            if((value.businessNames.length==0)&&(value.nodeNames.length==0)){
                                                                status.push('使用中');
                                                            }else {
                                                                status.push(
