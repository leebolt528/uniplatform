...
class DataSet extends Component{
    ...
    fetchDataTypes(){
        ...
        .then((data) => {
            ...
            this.setState({dataTypes},()=>this.clickDataTypes(0));
        }).catch((err) => console.log(err.toString()));
    }
     //点击语料类型
    clickDataTypes(index1){
        if(index1==1||index1==2){
            return;
        }
        let index=3;
        let {dataTypesIndex,dataTypes,dataTypeId,indexDataSet,showDataSetBool,indexObject,colorIndex,indexCorpus,pageStart,searchTableBool,sizeDataSet}=this.state;
        dataTypesIndex=index;
        dataTypeId=dataTypes[index].id;
        Object.keys(dataTypes).map((index) => 
           dataTypes[index].status=false
        );
        dataTypes[index1].status=true;
        indexDataSet=1;
        indexObject=1;
        colorIndex='Classify';
        indexCorpus=1;
        ...
        searchTableBool=false;
        sizeDataSet=9;
        this.setState({dataTypes,dataTypeId,dataTypesIndex,indexDataSet,showDataSetBool,indexObject,colorIndex,indexCorpus,pageStart,searchTableBool,sizeDataSet},()=>this.getFunctions());
    }
    //过滤分词语料功能
                        ...
                        <div key="">
                            <Col size={{ normal: 24, small: 24, medium: 24, large: 24 }} className="filter">
                                ...
                                <Input placeholder="请输入要查询的词" type="search"
                                    ...
                                    onClear={this.searchClearSet.bind(this)}>
                    ...
                    <div className="corpusManage_content">
                        <Col size={{ normal: 24, small: 24, medium: 24, large: 24 }} className="filter">
                            <Input placeholder="请输入要查询的词" type="search"
                                ...
                                onClear={this.searchClearCorpus.bind(this)}>
                                <Input.Left icon="search" />
                            </Input>
    ...
    //点击对象页面数据集
    clickInDataSet(){
        let {colorIndex,objectBool,indexObject,indexCorpus,pageStart,objectId,sizeCorpus,searchTableBool}=this.state;
        colorIndex='Classify';
        objectBool=false;
        indexObject=1;
        indexCorpus=1;
        ...
        this.setState({colorIndex,objectBool,indexObject,indexCorpus,pageStart,objectId,sizeCorpus,searchTableBool},()=>this.fetchObject());
    }
    //点击对象
    clickObject(index){
        let {colorIndex,objectId,objectsData,objectBool,indexCorpus,pageStart,pageBool,sizeCorpus,searchTableBool}=this.state;
        ...
        searchTableBool=false;
        this.resetSearchRule();
        this.setState({colorIndex,objectId,objectBool,indexCorpus,pageStart,pageBool,sizeCorpus,searchTableBool},()=>this.fetchCorpusObject(true));
    }
    //返回
    quitObjectShow(){
        let {colorIndex,showDataSetBool,sizeCorpus,searchTableBool,indexCorpus,pageStart}=this.state;
        colorIndex='Classify';
        showDataSetBool=true;
        sizeCorpus=10;
        searchTableBool=false;
        indexCorpus=1;
        pageStart=0;
        this.setState({colorIndex,showDataSetBool,sizeCorpus,searchTableBool,indexCorpus,pageStart},()=>this.fetchDataSetPage());
    }
    //添加数据集
    showAddDataSetModal(){
        this.fetchTypeFunc();
   ...
   /*  handleListChange( leftList, rightList, direction, moveList){
    ...
    render(){
        let {bodyHtml,editShowCorpusSet,addDataSetVisible,typeFunction,corpusSetData,corpusMaxNum,corpusTransferVisible,leftList,rightList,editDataSetVisible,editShowDataSet,objectsTransferData,objectId,objectBool,descriptionTransfer,higherLevelTransfer,titleTransfer,progressBool,dataTypes} = this.state;
        const listStyle = {
            width: '220px',
            height: '320px'
        };
                ...
                <Row style={{minHeight:'680px',margin:'0px'}}>
                    ...
                    <Col size={{ normal: 21, small: 24, medium: 21, large: 21 }} style={{padding:'0',marginTop:'20px'}}>
                            ...
                            <div className="corpusManage_content">
                                {
                                   dataTypes.map((value,index)=>{     
                                        return(
                                            <div key={index} style={{backgroundColor:`${dataTypes[index].status?'#ceeefc':'#fff'}`,border:`${dataTypes[index].status?'solid 1px #6fccf5':'1px solid #e4e4e4'}`}} 
                                                className="cortype hang" onClick={this.clickDataTypes.bind(this,index)}>
                                                ...
                                                <span className="cortype_text" style={{color:'#da8b5c'}}>
                                                    {dataTypes[index].name}<br/>({`${(index==0||index==3)?dataTypes[3].corpusNum:0}`})
                                                </span>
                                            </div>
                                        )
                                    })
                        ...
                        <Row>
                            <Col size={{ normal: 24, small: 24, medium: 24, large: 24 }} className="corpusManage_col">
                                <div className = "corpusManage_title" >
                                    <span className = "corpusManage_span">数据集列表</span>
                                </div>
                            </Col>
                            {bodyHtml}
                        </Row>
