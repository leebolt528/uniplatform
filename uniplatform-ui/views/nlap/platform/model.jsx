...
class ModelManage extends Component {
    ...
    showFunctions(){
        ...
        for(let i=0; i<algorithmsList.length; i++){
            if(i == 0){
                ...
                functionHtml.push(
                    <a key={i}>
                        <div style={{display:'inline-block',border:'1px solid #d8dde8',margin:'1.5%',borderRadius:'3px',width:'22%',height:'100px',cursor:'not-allowed'}}>
                            <Row style={{color:'#0070d2',fontSize:'18px',marginTop:'8%'}}>
                                <Col size={{ normal: 12, small: 24, medium: 12, large: 12 }} style={{textAlign:'right'}}>功能名称:</Col>
                                <Col size={{ normal: 12, small: 24, medium: 12, large: 12 }}>{algorithmsList[i].functionName}</Col>
                                <Col size={{ normal: 12, small: 24, medium: 12, large: 12 }} style={{textAlign:'right'}}>模型数量:</Col>
    ...
    handleOnClickAlgo(functionId,algorithmId,loc){
        ...
        this.setState({functionId,algorithmId,algorithmName,showModelsList:true});
        if(algorithmId == 'all'){
    ...
    afterEditSubmit(data){
        ...
        if(data.code){
            ...
            if(data.status == 200){
                ...
                if(algorithmId_echo == 'all'){
                    this.getFetchModelsList(functionId,'');
                }else{
                    this.getFetchModelsList('',algorithmId_echo);
                }
            }
        }
    }
                ...
                for(let j=0; j<algo.length; j++){
                    ...
                    algorithmHtml.push(
                        <a onClick={this.handleOnClickAlgo.bind(this,algo[j].functionId,algo[j].id,j)} key={j}>
                            <div className="algo_select" style={{display:'inline-block',border:'1px solid #d8dde8',margin:'1%',borderRadius:'3px',width:'18%',height:'100px'}}>
    ...
    backFunction(){
        ...
        this.setState({showAlgorith : false, showModelsList : false});
    }
    ...
    render() {
        let {functionHtml,algorithmHtml,showAlgorith,modelsList,selectedModel,index,size,total,addModelVisible,
            ...
            algorithmId_echo,importVisible,selectedInUsing,selectedOperation,selectedStatus,algorithmList,noData,
            showModelsList} = this.state;
        return (
            <Page>
