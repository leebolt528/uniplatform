...
class AutoWordSegment extends Component {
    constructor(props){
        super(props);
    ...
    handleValue() {
        ...
        if(!text.replace(/(^\s*)|(\s*$)/g, "")){
            popup(<Snackbar message="请填写需要分词的文本"/>);
            return;
        }
        let show = true;
        this.setState({show});
        ...
        if(flag == 'chinese'){
            fetch(Uniplatform.context.url + '/nlap/admin/segment/auto/segment', {
            ...
            fetch(Uniplatform.context.url + '/nlap/admin/segment/auto/segment/en', {
                credentials: 'same-origin',
                method: 'POST',
                body: param
            })
    ...
    showSegment(){
        ...
        words.map(function (item) {
            all_natures.push(item.nature);
        });
        let natures = [...new Set(all_natures)]

        for(let i=0; i<natures.length; i++){
            natureHtml.push(
                <div key={natures[i]} style={{display:'inline-block',margin:'2px 10px',padding:'4px',backgroundColor:color[i]}}
    ...
    clickNature(nature){
        ...
        let wordHtml = [];
        if(nature == this.state.nature){
            if(this.state.wordHtml.length == words.length){
                for(let i=0; i<words.length; i++){
                    if(words[i].nature == nature){
                        let colorIndex;
                        natures.map(function (item,index) {
                            if(item == words[i].nature){
                                colorIndex = index;
            ...
            <Page>
                ...
                <Row style={{minHeight:'680px'}}>
                    <Col size={{ normal: 3, small: 24, medium: 3, large: 3 }} style={{marginTop:'-5px',padding:'0',backgroundColor:'#fff',height:'880px',zIndex:'21'}}>
                        ...
                        <div>
                            <ProductMenu url={'/nlap/product/autoWordSegment'}/>
                        </div>
                    </Col>
                    <Col size={{ normal: 21, small: 24, medium: 21, large: 21 }} style={{padding:'0'}}>
                        <Row style={{margin:'10px 20px'}}>
                            <img src="../image/in1.png"/><span className="classTitle">自动分词</span>
                        </Row>
                        <Row className="enterContent" style={{margin:'0 20px'}}>
                            ...
                            <Button type="info" style={{float:'right',marginRight:'30px'}} onClick={this.handleValue.bind(this)}>分词</Button>
                        ...
                        <Row className="dicManage_row">
                            <Col size={{ normal: 24, small: 24, medium: 24, large: 24 }} className="dicManage_col">
                                <div className = "dicManage_title" >
                                    <span className="dicManage_span">分词结果</span>
                                </div>
                            </Col>
                            <div className="dicManage_content" style={{maxHeight:'500px',overflow:'auto'}}>
                                <Col size={{ normal: 24, small: 24, medium: 24, large: 24 }} style={{margin:'20px 23px',width:'95%'}}>
                                    ...
                                    <Divider line />
                                    <span style={{fontSize:'12px'}}>{show ? (natureHtml ? natureHtml : <Loading />) : null}</span>
                                </Col>
                            </div>
                        </Row>
                    </Col>
...
AutoWordSegment.UIPage = page;
export default AutoWordSegment;