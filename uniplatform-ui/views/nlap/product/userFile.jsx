...
class UserFile extends Component {
    ...
    handleValue() {
        ...
        fetch(Uniplatform.context.url + '/nlap/admin/text/classify/file', {
        ...
        for(let i=0;i<classes.length;i++){
            claHtml.push(
                <span key={i} style={{backgroundColor:'#a3db5b',padding:'2px',margin:'2px'}}>
                    {classes[i].className} : {classes[i].accuracy.toFixed(2)}
                </span>
            );
        }
        return claHtml;
            ...
            <Page>
                ...
                <Row style={{minHeight:'680px'}}>
                    <Col size={{ normal: 3, small: 24, medium: 3, large: 3 }} style={{marginTop:'-5px',padding:'0',backgroundColor:'#fff',height:'880px',zIndex:'21'}}>
                        ...
                        <div>
                            <ProductMenu url={'/nlap/product/userFile'}/>
                        </div>
                    </Col>
                    <Col size={{ normal: 21, small: 24, medium: 21, large: 21 }} style={{padding:'0'}}>
                        <Row style={{margin:'10px 20px'}}>
                            <img src="../image/in8.png"/><span className="classTitle">文本分类-用户文件</span>
                        </Row>
                        <Row className="enterContent" style={{margin:'0 20px',height:'150px',overflow:'auto'}}>
                                    ...
                                    {classifier ?
                                        <Table striped={true} multiLine={true} dataSource={classifier} bodyScrollable bodyHeight={'570px'}>
                                            ...
                                            <Column title="分类结果" scaleWidth='40%' textAlign="center">
                                                {(value)=>{
                                                    return this.showClasses(value.classes);
                                                }}
                                            </Column>
                                        </Table>
                                        : null
