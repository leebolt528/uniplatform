...
class PinyinCallouts extends Component {
    constructor(props){
        super(props);
    ...
    handleValue() {
        this.setState({pinyin:undefined});
        const text = this.getValue();
        if(!text.replace(/(^\s*)|(\s*$)/g, "")){
            popup(<Snackbar message="请填写需要标注的文本"/>);
            return;
        }
        let show = true;
        this.setState({show});
        ...
        fetch(Uniplatform.context.url + '/nlap/admin/pinyin/tagging', {
            ...
            .then((data) => {
                if(data.code){
                    ...
                    let pinyin = data.data.replace(/([a-zA-Z]|ü)+\d\b/g,function(word){
            ...
            <Page>
                ...
                <Row style={{minHeight:'680px'}}>
                    <Col size={{ normal: 3, small: 24, medium: 3, large: 3 }} style={{marginTop:'-5px',padding:'0',backgroundColor:'#fff',height:'880px',zIndex:'21'}}>
                        ...
                        <div>
                            <ProductMenu url={'/nlap/product/pinyinCallouts'}/>
                        </div>
                    </Col>
                    <Col size={{ normal: 21, small: 24, medium: 21, large: 21 }} style={{padding:'0'}}>
                        <Row style={{margin:'10px 20px'}}>
                            <img src="../image/in5.png"/><span className="classTitle">拼音标注</span>
                        </Row>
                        <Row className="enterContent" style={{margin:'0 20px'}}>
                            ...
                            <Button type="info" style={{float:'right',marginRight:'30px'}} onClick={this.handleValue.bind(this)}>标注汉语拼音</Button>
                        </Row>
                        <br/>
                        <Row className="dicManage_row">
                            <Col size={{ normal: 24, small: 24, medium: 24, large: 24 }} className="dicManage_col">
                                <div className = "dicManage_title" >
                                    <span className="dicManage_span">标注结果</span>
                                </div>
                            </Col>
                            <div className="dicManage_content" style={{maxHeight:'500px',overflow:'auto'}}>
                                <Col size={{ normal: 24, small: 24, medium: 24, large: 24 }} style={{margin:'20px 23px',width:'95%'}}>
                                    <span style={{fontSize:'12px'}}>
                                        {show ? (pinyin ? pinyin : <Loading />) : null}
                                    </span>
                                </Col>
                            </div>
                        </Row>
...
PinyinCallouts.UIPage = page;
export default PinyinCallouts;