            ...
            <Page>
                ...
                <Row>
                    <Col size={{ normal: 4, small: 24, medium: 4, large: 4 }}>
                        <UsouMenu url={'/usou/monitorManage_Rule'} id={this.props.page.id} name={this.props.page.name}/>
                    </Col>
                    <Col size={{ normal: 20, small: 24, medium: 20, large: 20 }} style={{minHeight: '600px'}}>
                        <Row>
                            <NavigationBar code={'monitorManage_Rule'+this.props.page.name} innerId={this.props.page.id} name={this.props.page.name}/>
                        </Row>
                        <div >
                            <Row>
                                <ButtonGroup key="features">
                                    <Button type="default" size="medium" onClick={this.addMonitor}>添加预警</Button>
                                    <Button type="default" size="medium" onClick={this.confirmBatchDelete}>批量删除</Button>
                                </ButtonGroup>

                            </Row>

                            <Row>
                                <Table dataSource={ dataSource } checkable onCheck={ this.handleSelect } striped={ true } multiLine={ true } >
                                    <Column title="预警名称" dataIndex="name" scaleWidth = '20%' textAlign="center"/>
                                    <Column title="预警规则" dataIndex="threshold" scaleWidth = '20%' textAlign="center"/>
                                    <Column title="预警方式" dataIndex="mode"  scaleWidth = '20%' textAlign="center"/>
                                    <Column title="预警联系方式" dataIndex="contact"  scaleWidth = '20%' textAlign="center"/>

                                    <Column title="操作" scaleWidth = '20%' textAlign="center">
                                        { ( value ) => {
                                            return (
                                                <div>
                                                    <Button type="default" size="tiny" onClick={this.alterMonitor.bind(this,value.id)}>
                                                        <Icon icon="edit" /> 修改
                                                    </Button>
                                                    <Button type="default" size="tiny" onClick={this.confirmDelete.bind(this,value.id)}>
                                                        <Icon icon="trash-o" /> 删除
                                                    </Button>
                                                    <Button type="default" size="tiny" onClick={this.getMonitorResult.bind(this,value.id)}>
                                                        <Icon icon="trash-o" /> 查看结果
                                                    </Button>
                                                </div>
                                            )
                                        } }
                                ...
                                <Divider />
                                <Pagination name="pages"  index={ index }  total={ total>1000?1000:total } size={ size } align='right'
