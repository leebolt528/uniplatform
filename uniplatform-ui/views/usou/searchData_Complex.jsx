...
class SearchData extends Component {
    ...
    //接收复合查询返回值
    afterComplexSubmit(complexData){
       let complexShow=true;
       if(complexData.code){
            let  complexError =complexData.data;
            this.setState({complexError,complexData,complexShow});
        }else{   
            if(complexData.search.hits.hits.length==0){
                let  complexError ="No result";
                this.setState({complexError});
            }
            let { search } = complexData;
            this.setState( { search,complexData,complexShow} );
            this.getQueryDataSource();
            this.getQueryDataColumn();
        }
    }
            ...
            <Page>
                ...
                <Row style={{minHeight: '600px'}}>
                    <Col size={{ normal: 4, small: 24, medium: 4, large: 4 }}>
                        <UsouMenu url={'/usou/searchData_Complex'} id={this.props.page.id} name={this.props.page.name}/>
                    </Col>
                    <Col size={{ normal: 20, small: 24, medium: 20, large: 20 }}>
                        <Row>
                            <NavigationBar code={'searchData_Complex'+this.props.page.name} innerId={this.props.page.id} name={this.props.page.name}/>
                        </Row>
                        <Row>
                            <Col size={{ normal: 24, small: 24, medium: 6, large: 6 }} style={{paddingLeft:'0px'}}>
                                <Form
                                action={ `${contextUrl}/usou/cluster/${this.props.page.id}/_search/complex` }
                                method="post"
                                type="horizontal"
                                async={ true }
                                className="searchForm" 
                                onSubmit={ this.beforeComplexSubmit }
                                onAfterSubmit={this.afterComplexSubmit}
                                >
                                    <FormItem>
                                        <Input name="route" value="_search" />
                                    </FormItem>
                                    <FormItem>
                                        <Select value={ 'POST' } name="method">
                                    ...
                                    <FormItem>
                                        <Textarea rows={ 20 } name="body" value={body} onChange={this.textChange.bind(this,"textVal")}/>
                                    </FormItem>
                                    <FormItem style={{display:'inline-block',float:'left',width:'70px'}}>
                                        <Checkbox value="pretty" name="pretty" onChange={this.getPretty.bind(this)}>易读</Checkbox>
                                    </FormItem>
                                    <ButtonGroup style={{float:'right'}}>
                                        <Button type="default" size="tiny" onClick={this.stringToJson.bind(this)}>验证JSON</Button>
                                        <Button type="default" size="tiny" htmlType="submit">提交请求</Button>
                                    </ButtonGroup>
                                </Form>
                            </Col>
                                ...
                                }
                                {
                                    complexShow?
                                        ...
                                        <Col size={{ normal: 24, small: 24, medium: 17, large: 17 }} style={{paddingLeft:'0px'}} className="sqlError">
                                            <Divider/>
                                            <Alert message={complexError} type="error" />
                                        </Col>  
                                        :
                                            Object.keys(resultSearch).length > 0 ? 
                                            <Table textAlign="center" dataSource={resultSearch} columns={resultColumn} striped={ true } bordered={ true } checkable headBolder complex onCheck={ this.handleSelect } style={{display:'inline-block'}}/>
                                            : 
                                            <Col size={{ normal: 24, small: 24, medium: 17, large: 17 }} style={{paddingLeft:'0px'}} className="sqlError">
                                                <Divider/>
                                                <Alert message={complexError} type="info" />
                                            </Col> 
                                    :
                                    ''
                                }
