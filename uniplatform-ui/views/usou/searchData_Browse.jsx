...
class SearchData extends Component {
    ...
    fetchSearchData(){
        fetch( Uniplatform.context.url + '/usou/cluster/' + this.props.page.id + '/_search', {
            ...
            .then( ( data ) => {
                ...
                let browseShow=true;
                this.setState( { search, state,browseShow } );
                this.getIndicesName();
    ...
    addIndiceNameClass(index){
		...
		this.setState({indicesName ,panelIndice,panelType,indicesType,browseShow});
        this.getIndicesTable(panelIndice,panelType,panelField);
    }
    //索引下类型点击事件
    addIndiceTypeClass(index){
		...
		this.setState({indicesType,panelType,indicesName,panelIndice,browseShow});
        this.getIndicesTable(panelIndice,panelType,panelField);
    }
    //索引下字段点击事件
    addIndiceFieldClass(index){
		...
		this.setState({indicesField: indicesField,browseShow});
    }
    //监控索引下字段值
    ...
    render() {
        let {took,total,basicSearchShow,resultsql,sqlIndex,basicIndex,basicSize,queryCount,sqlError,explainShow,sqlShow,
            aliasName,indicesName,indicesType,indicesField,resultSearch,resultColumn,body,updateVisible,panelIndice,
            panelType,panelField,source,sourceMessage,resultBasic,selectSource,sqlVal,sqlExplain,browseShow} = this.state;
        return (
            <Page>
                ...
                <Row style={{minHeight: '600px'}}>
                    <Col size={{ normal: 4, small: 24, medium: 4, large: 4 }}>
                        <UsouMenu url={'/usou/searchData_Browse'} id={this.props.page.id} name={this.props.page.name}/>
                    </Col>
                    <Col size={{ normal: 20, small: 24, medium: 20, large: 20 }}>
                        <Row>
                            <NavigationBar code={'searchData_Browse'+this.props.page.name} innerId={this.props.page.id} name={this.props.page.name}/>
                        </Row>
                       <Row>
                            <Col size={{ normal: 24, small: 24, medium: 6, large: 6 }} style={{paddingLeft:'0px'}}>
                                <Select required value = {'_all'} dataSource={aliasName}  onChange={this.fetchSearchIndice}/>
                                <Accordion unique={false}  expandedPanel={[0,1,2]}>
                                    <Accordion.Panel title="索引">
                                        {
                                            indicesName.map( (item, index) => {
                                                return (<div onClick={this.addIndiceNameClass.bind(this,index)} className={`accordion_content${ item.visibility ? ' selected' : '' }`} key={index}>{item.value}</div>)
                                            })
                                        }
                                    </Accordion.Panel>
                                    <Accordion.Panel title="类型">
                                        {
                                            indicesType.map( (item, index) => {
                                                return (<div onClick={this.addIndiceTypeClass.bind(this,index)} className={`accordion_content${ item.visibility ? ' selected' : '' }`} key={index}>{item.value}</div>)
                                            })
                                        }
                                    </Accordion.Panel>
                                    <Accordion.Panel title="字段">
                                        {
                                            indicesField.map( (item, index) => {
                                            return (
                                                <div key={index} className="accordion_input">
                                                    <div onClick={this.addIndiceFieldClass.bind(this,index)} className="accordion_content">{item.visibility ? <Icon icon="angle-down"/> : <Icon icon="angle-right"/>}  {item.value}</div>
                                                    <div style={{display: `${item.visibility ? 'block':'none'}`}}><Input type="text" name={item.value} onChange={this.changeIndeciField.bind(this,index,"fieldVal")}/></div>
                                                </div>
                                                )
                                            })
                                        }
                            ...
                            <Col size={{ normal: 24, small: 24, medium: 18, large: 18 }}>
                                <ButtonGroup style={{float:'right',position:'relative',marginTop:'10px',zIndex:1}}>
                                ...
                                <Divider/> 
                                {
                                    browseShow?
                                        ...
                                        <Table textAlign="center" striped={true} checkable headBolder bordered={ true } bodyScrollable={ true } bodyHeight={ '1000px' }
                                            dataSource={resultSearch} columns={resultColumn} onCheck={ this.handleSelect }/>
                                        :
                                        <Col size={{ normal: 24, small: 24, medium: 17, large: 17 }} style={{paddingLeft:'0px'}} className="sqlError">
                                            <Divider/>
                                            <Alert message={"No result"} type="info" />
                                        </Col> 
                                    :
                                    ''
                                }
